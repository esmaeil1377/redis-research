...
برای ساخت و اضافه کردن یک عنصر به set از دستور SADD استفاده می کنیم.خروجی این دستور یا یک است که نشان دهنده ی اضافه شدن عنصر در مجموعه است. یا صفر است که به معنی تکراری بودن عنصر در مجموعه است. به عنوان مثال در زیر
...
مثال جالب Redis Pub/Sub در این زمینه این است که  Redis مانند تلوزیون که یک فیلم PUBLISH میکند و یک بیننده اون فیلم را SUBSCRIBE میکند و اگر بیننده وجود نداشته باشد همه اطلاعات ازبین خواهد رفت.
...
### حال به بررسی چند دستور پایه و مهم می‌پردازیم:
* تنظیم یک مقدار:
<div dir = 'ltr'>
...
```
example: SET firstname Albert EX 10 NX
```  
</div>
...
در مثال بالا کلید شمارنده در هنگام شروع مقدار 4 را خواهد داشت، و پس از دستور دوم، به 10 افزایش خواهد یافت.
...
### دستورات دیگر:
* تعریف یک کلید با مدت اعتبار:
<div dir = 'ltr'>
  
```
SETEX <key> 40 "I said, Hello World!"
```  
</div>

کلید با 40 ثانیه انقضا تنظیم شده است.

* دریافت زمان باقی مانده برای کلیدی که مدت اعتبار خاصی دارد:
<div dir = 'ltr'>
...
* تنظیم یک مقدارتبدیل یک کلید از اعتباری به دائمی:
<div dir = 'ltr'>
...
* تغییر نام کلید از پیش تعریف شده:
<div dir = 'ltr'>
...
تغییر نام key1 به key2

* حذف هر آن چیزی که تا به حال ذخیره شده:
<div dir = 'ltr'>
...
شما میتوانید از این [repo](https://github.com/divyanshutomar/hello-redis) استفاده کرده و یک پروژه ساده با redis را بالا بیاورید.

## استفاده از Client برای زبان های مختلف:
تقریبا تمامی زبان ها دارای client ای برای ردیس هستند که در این [لینک](https://redis.io/clients) می توانید بعضی از آن ها را مشاهده کنید.
بنا براین با هر زبان برنامه نویسی به راحتی می توان به دیتا بیس Redis متصل شد و دستورات ردیس را اجرا کرد.
بعد از نصب Redis  می توانید با زدن دستور زیر به redis cli وارد شوید و آن را بررسی نمایید:
...
بعد از اجرای دستور بالا به محیط اجرای دستورات redis وارد می شویم
